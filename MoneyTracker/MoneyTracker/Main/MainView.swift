...
struct MainView: View {
    ...
    @State private var shouldPresentChart = false
    
    @Environment(\.managedObjectContext) private var viewContext

    @FetchRequest(
        sortDescriptors: [NSSortDescriptor(keyPath: \Card.timestamp, ascending: false)],
        predicate: NSPredicate(format: "user == %@", LoggedInUser.value),
        animation: .default)
    ...
    var body: some View {
        NavigationView {
            ScrollView {
                ...
                Spacer()
                    .fullScreenCover(isPresented: $shouldPresentChart, onDismiss: nil) {
                        if let firstIndex = cards.firstIndex(where: {$0.hash == selectedCardHash}) {
                            let card = self.cards[firstIndex]
                            Chart(card: card)
                        }
