...
struct AddTransactionForm: View {
    let card: Card
    let transactionsRequest: FetchRequest<CardTransaction>
    ...
    init(card: Card) {
        ...
        transactionsRequest = FetchRequest<CardTransaction>(entity: CardTransaction.entity(), sortDescriptors: [
    ...
    @ObservedObject private var nameValidator = TextValidator(
        validate: { _ in return true },
        validationText: ""
    )

    @ObservedObject private var amountValidator = TextValidator(
        validate: { _ in return true },
        validationText: ""
    )
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Information")) {
                    TextField("Name", text: $nameValidator.text)
                    TextField("Amount", text: $amountValidator.text)
                ...
                .alert("Transaction amount is larger than credit limit that is left", isPresented: $showingAlert) {
    ...
    private var saveButton: some View {
        Button {
            let context = PersistenceController.shared.container.viewContext
            let amount = Float(self.amountValidator.text) ?? 0
            let balance = transactionsRequest.wrappedValue.reduce(0, {$0 + $1.amount })
            if Float(self.card.limit) - balance - amount < 0 {
                self.showingAlert = true
            } else {
                let transaction = CardTransaction(context: context)
                transaction.name = self.nameValidator.text
        ...
        .disabled(!self.amountValidator.isValid || !self.nameValidator.isValid)
    }
